...
import { EPaymentDirection, EPaymentStatus } from "src/modules/payments/common/enums/core";
import { formatDecimalString, setPlatformId } from "src/common/utils";
import { ESequenceName } from "src/common/enums";
import { TConstructPaymentDto, TCreatePaymentRecordPayment } from "src/modules/payments/common/types/management";
...
export class PaymentsManagementService {
  ...
  public async createPaymentRecord(
    ...
    if (existingPayment) {
      const paymentPartial: DeepPartial<Payment> = {
        ...existingPayment,
        totalAmount: formatDecimalString(existingPayment.totalAmount),
        totalGstAmount: formatDecimalString(existingPayment.totalGstAmount),
        totalFullAmount: formatDecimalString(existingPayment.totalFullAmount),
      };
      payment = manager.getRepository(Payment).create(paymentPartial);
    } else {
      payment = await this.constructAndCreatePayment(manager, data);
    }

    const paymentItem = await this.constructAndCreatePaymentItem(manager, payment.id, data);
   ...
   *
   * @param manager - The TypeORM EntityManager for transactional operations.
   * @param paymentId - The existing or newly created Payment entity id to associate the item with.
  ...
  public async constructAndCreatePaymentItem(
    ...
    const newPaymentItemDto = this.constructPaymentItemDto(data, paymentId);
  ...
  private async generatePaymentPlatformId(data: ICreatePaymentRecord): Promise<string> {
    ...
    if (appointment && data.direction !== EPaymentDirection.OUTCOMING) {
  ...
  private async constructPaymentDto(data: TConstructPaymentDto): Promise<IPayment> {
    ...
    const determinedToInterpreter = data.toInterpreter ? (data.toInterpreter as UserRole) : null;
    ...
    return {
      platformId,
      totalAmount: formatDecimalString(prices?.clientAmount ?? prices?.interpreterAmount ?? 0),
      totalGstAmount: formatDecimalString(prices?.clientGstAmount ?? prices?.interpreterGstAmount ?? 0),
      totalFullAmount: formatDecimalString(prices?.clientFullAmount ?? prices?.interpreterFullAmount ?? 0),
      estimatedCostAmount: formatDecimalString(prices?.clientFullAmount ?? 0),
      ...
      toInterpreter: determinedToInterpreter,
  ...
  private constructPaymentItemDto(data: ICreatePaymentRecord, paymentId: string): IPaymentItem {
    const { prices } = data;

    return {
      amount: formatDecimalString(prices?.clientAmount ?? prices?.interpreterAmount ?? 0),
      gstAmount: formatDecimalString(prices?.clientGstAmount ?? prices?.interpreterGstAmount ?? 0),
      fullAmount: formatDecimalString(prices?.clientFullAmount ?? prices?.interpreterFullAmount ?? 0),
      currency: data.currency,
      status: data.status,
      note: data.note ?? null,
      ...
      amountOfAppliedDiscountByMembershipMinutes: prices?.appliedDiscounts?.membershipFreeMinutes
        ? formatDecimalString(prices.appliedDiscounts.membershipFreeMinutes)
        : null,
      amountOfAppliedDiscountByMembershipDiscount: prices?.appliedDiscounts?.membershipDiscount
        ? formatDecimalString(prices.appliedDiscounts.membershipDiscount)
        : null,
      amountOfAppliedDiscountByPromoCode: prices?.appliedDiscounts?.promoCampaignDiscount
        ? formatDecimalString(prices.appliedDiscounts.promoCampaignDiscount)
        : null,
