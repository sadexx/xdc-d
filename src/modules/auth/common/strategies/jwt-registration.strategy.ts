...
@Injectable()
export class JwtRegistrationStrategy extends PassportStrategy(Strategy, TokenStrategies.JWT_REGISTRATION_STRATEGY) {
  constructor(protected readonly configService: ConfigService) {
    super({
      jwtFromRequest: ExtractJwt.fromExtractors([
        ExtractJwt.fromAuthHeaderAsBearerToken(),
        (request: Request): any => request.cookies?.[ETokenName.REGISTRATION_TOKEN],
      ]),
      secretOrKey: configService.getOrThrow<string>("JWT_REGISTRATION_TOKEN_SECRET"),
    });
  }
