...
} from "src/modules/payments/common/types/transfer";
import {
  EPaymentCurrency,
...
import { ICalculatePaymentGroupTotals } from "src/modules/payments/common/interfaces/transfer";
...
export class PaymentsCorporatePayoutService {
  ...
  /**
   * Processes automated corporate payouts in batch.
   *
   * Finds all payments waiting for payout, groups by company, processes each company's
   * payments in a transaction, and generates receipt and tax invoice PDFs. Used for
   * scheduled batch processing of corporate interpreter payments.
   *
   * @returns Promise that resolves when all payouts are processed
   */
  ...
  private calculateGroupTotals(payments: TMakeCorporatePayOuts[]): ICalculatePaymentGroupTotals {
    ...
    for (const payment of payments) {
      const amount = parseDecimalNumber(payment.totalFullAmount ?? 0);
