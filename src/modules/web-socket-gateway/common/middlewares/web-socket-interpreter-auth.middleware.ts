...
const lokiLogger = new LokiLogger("WebSocketAuthInterpreterMiddleware");

export const WebSocketAuthInterpreterMiddleware = (jwtAccessService: JwtAccessService) => {
  return (client: Socket, next: (err?: any) => void): void => {
    ...
    try {
      ...
      if (!isInRoles(INTERPRETER_ROLES, payload.userRole as EUserRoleName)) {
        lokiLogger.error(
