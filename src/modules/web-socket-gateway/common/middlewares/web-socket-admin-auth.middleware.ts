...
const lokiLogger = new LokiLogger("WebSocketAdminAuthMiddleware");

export const WebSocketAdminAuthMiddleware = (jwtAccessService: JwtAccessService) => {
  return (client: Socket, next: (err?: any) => void): void => {
    ...
    try {
      ...
      if (!isInRoles(ADMIN_ROLES, payload.userRole as EUserRoleName)) {
        lokiLogger.error(
