...
import { IPaymentCalculationResult, IPaymentOperationResult } from "src/modules/payments-new/common/interfaces";
import { LokiLogger } from "src/common/logger";
import { PaymentsManagementService } from "src/modules/payments-new/services";
...
export class PaymentsCorporateSameCompanyCommissionService {
  ...
  public async processSameCompanyCommission(
    ...
    try {
      ...
      throw new InternalServerErrorException("Failed to process same company commission.");
    }
  }
  ...
  private async createRefundItem(manager: EntityManager, context: TProcessSameCompanyCommissionContext): Promise<void> {
    ...
    await this.paymentsManagementService.constructAndCreatePaymentItem(manager, payment as Payment, {
  ...
  private async updatePaymentTotals(
    ...
    await this.paymentsManagementService.updatePayment(
      ...
      { id: payment.id },
      {
        totalAmount: commissionAmounts.commissionWithoutGst,
        totalGstAmount: commissionAmounts.commissionGstAmount,
        totalFullAmount: commissionAmounts.commissionAmount,
  ...
  private async updateCompanyDeposit(
    ...
    const newDeposit = round2(currentDeposit + commissionAmounts.refundAmount);

    await manager.getRepository(Company).update(payment.company.id, { depositAmount: newDeposit });
  }
  ...
  private async updateAppointmentPaidByClient(
    ...
    await manager.getRepository(Appointment).update(appointment.id, {
      paidByClient: commissionAmounts.commissionAmount,
