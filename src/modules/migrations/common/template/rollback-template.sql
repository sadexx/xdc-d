-- Template: Rollback DDL stored in a variable, then record it in migration_logs.
...
DECLARE
  ...
  v_migration_type text := 'rollback';                          -- enum label used in your table (string)
  v_notes text := 'Rollback applied via pgAdmin Query Tool';    -- free-form notes
  -- -----------------------------------------
...
ALTER TABLE "promo_campaigns" DROP COLUMN IF EXISTS "conditions_url";
ALTER TABLE "promo_campaigns" DROP COLUMN IF EXISTS "banner_display";
...
BEGIN
  ...
  BEGIN
    ...
    RAISE NOTICE 'Rollback % executed and registered successfully.', v_migration_file_name;
